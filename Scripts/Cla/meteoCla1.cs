...
public class meteoCla1 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        corazones[corazones.Length - 1].enabled = false;
    ...
    private void ProcesarDanio(GameObject enemigo)
    {
        ...
        if (vida <= 0)
        {
            ...
            anim.SetTrigger("triggerMuere");
            StartCoroutine(MorirYDestruir());
        }
        else
        {
            anim.SetTrigger("triggerExplota"); // ← Solo si NO muere
        }
    ...
    private void ProcesarCuracion(GameObject objetoVida)
    {
        ...
        {
            vida += 1;
            corazones[corazones.Length - 1].enabled = true;
        }
        else {
            for (int i = vida; i < corazones.Length; i++)
            {
                corazones[i-1].enabled = true;
            }
